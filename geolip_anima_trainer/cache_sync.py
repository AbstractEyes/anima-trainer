...
DEFAULT_MODEL = "anima"
# Periodic pushes are CACHE-ONLY (the frozen images are uploaded once at freeze + final),
# so each push hashes/uploads only the growing cache subtree, not the whole dataset.
CACHE_ONLY_GLOB = ["**/cache/" + DEFAULT_MODEL + "/**"]
# Never sync the HF hub cache or lock files even if they land under the tree.
_DEFAULT_IGNORE = ["**/hf_cache/**", "**/.cache/**", "*.lock"]
